  ...
  return (
    <div
      ...
      className={`${className} transition-all ${
        isOver ? 'ring-2 ring-blue-500/50 border-blue-500/40 bg-blue-500/10' : ''
      }`}
    >
      {children}
    </div>
...
const DraggablePlayer: React.FC<{ player: Player; onToggleLock: (id: string) => void }> = ({
  player,
  onToggleLock,
}) => {
...
export const TeamBuilder: React.FC<GameProps> = ({
  ...
  const handleRandomAssign = () => {
    const unassignedUnlocked = players.filter((p) => p.teamId === null && !p.isLocked);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold tracking-tight text-white flex items-center gap-2">
                팀 빌더 <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20">Team Maker</span>
              </h2>
              <p className="text-[10px] sm:text-xs text-slate-400">참여자들을 균등하고 고르게 팀으로 나누어 배치합니다.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4 relative z-10">
            ...
            <div className="bg-slate-950/80 border border-slate-850 rounded-xl p-2.5 flex items-center justify-between gap-3 h-full">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[9px] uppercase font-bold tracking-wider text-slate-500">배정할 팀 개수</p>
                  <p className="text-xs font-semibold text-slate-200">{teamCount}개 팀</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                ...
                <span className="w-5 text-center text-xs font-bold text-white">{teamCount}</span>
                <button
        ...
        <div className="flex-1 relative z-10 flex flex-col lg:flex-row gap-5 overflow-hidden h-full">
          {isStarted ? (
              ...
              <div className="w-full lg:w-1/3 flex flex-col bg-slate-950 border border-slate-800 rounded-xl p-3 sm:p-4 overflow-hidden h-full max-h-[200px] lg:max-h-none">
                <div className="flex items-center justify-between mb-2 border-b border-slate-850 pb-1.5 flex-shrink-0">
                  <h3 className="text-xs font-bold text-slate-400 flex items-center gap-1.5">
                    대기 명단 <span className="text-[10px] bg-slate-900 text-slate-500 px-1.5 py-0.5 rounded-md font-mono">{unassignedPlayers.length}</span>
                  </h3>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between border-b border-white/5 pb-1.5 mb-1 flex-shrink-0">
                          <span className="text-xs font-bold text-white tracking-wide">{team.name}</span>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/50 backdrop-blur-[2px] p-4 text-center z-20">
              ...
              <h3 className="text-lg font-bold text-white mb-1">팀 빌더 준비 완료</h3>
              <p className="text-sm text-slate-400 max-w-sm">
                왼쪽 참여자 명단에 이름을 입력한 뒤 아래 <strong className="text-blue-400">🚀 추첨 시작</strong> 버튼을 누르세요!
              </p>
              <div className="mt-4 text-[10px] sm:text-xs font-mono text-slate-500 border border-slate-800 bg-slate-900/40 py-1.5 px-3 rounded-md">
                드래그 앤 드롭으로 수동 배치하거나 자물쇠를 활용해 명단을 고정할 수 있습니다.
              </div>
            </div>
          )}
        </div>
          ...
          ? createPortal(
              <DragOverlay>
                {activeId && activeDraggedPlayer ? (
                  ...
                  >
                    <span className="truncate pr-2">{activeDraggedPlayer.name}</span>
                    <div
